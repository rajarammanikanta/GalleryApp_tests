...
class Gallery extends Component {
  ...
  getFilteredImages = () => {
    ...
    const filteredImageList = imagesList.filter(
      eachList => eachList.id === activeImage,
    )
    ...
    return (
      <div>
        <div className="main-container">
          <ul>
            {filteredImageList.map(eachList => (
              <li>
                <img
                  src={eachList.imageUrl}
                  alt={eachList.imageAltText}
                  className="image1"
                />
              </li>
            ))}
          ...
          <p>Nature Photography by Rajaram Manikanta</p>
          <div>
            <ul className="thumbnail-container">
              {imagesList.map(eachList => (
                <ThumbnailItem
                  eachList={eachList}
                  key={eachList.id}
                  updateImageByClick={this.updateImageByClick}
                />
              ))}
            </ul>
          </div>
